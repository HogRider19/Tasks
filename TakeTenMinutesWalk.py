...
with a Walk Generating App on their phones -- everytime you press the button it sends you an array of one-letter strings 
...
def is_valid_walk(walk: list):
    direction_map = {'n': 1, 's': -1, 'e': 1j, 'w': -1j}        
    return not sum([direction_map[direct] for direct in walk]) and len(walk) == 10
